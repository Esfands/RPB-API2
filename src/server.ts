      ...
      if (true === verifyMessage(hmac, req.headers[TWITCH_MESSAGE_SIGNATURE])) {
        ...
        if (MESSAGE_TYPE_NOTIFICATION === req.headers[MESSAGE_TYPE]) {
          ...
          if (notification.subscription.type === "stream.online") {
            ...
            StreamStat.findOneAndUpdate({ type: "esfandtv" }, { status: "live" })
              .then(res => console.log("EsfandTV just went live..."));

          } else if (notification.subscription.type === "stream.offline") {
            StreamStat.findOneAndUpdate({ type: "esfandtv" }, { status: "offline", hosting: "" })
              .then(res => console.log("EsfandTV just went offline..."));

          } else if (notification.subscription.type === "channel.update") {
            StreamStat.findOneAndUpdate({ type: "esfandtv" }, { title: notification.event.title, category: notification.event.category_name })
